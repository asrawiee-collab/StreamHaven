                        ...
                        HStack(spacing: 20) {
                            ForEach(watchHistory) { history in
                                if let movie = history.movie {
                                    Button(action: { navigationCoordinator.goTo(.movieDetail(movie)) }) {
                                    ...
                                    Button(action: { navigationCoordinator.goTo(.seriesDetail(series)) }) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 20) {
                        ForEach(series) { seriesItem in
                            Button(action: { navigationCoordinator.goTo(.seriesDetail(seriesItem)) }) {
