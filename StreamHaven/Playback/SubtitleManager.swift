...
public final class SubtitleManager: ObservableObject, SubtitleManaging {
    ...
    /// Initializes a new `SubtitleManager` and loads the API key from the Keychain if not injected.
    public init(apiKey: String? = nil) {
        if let apiKey = apiKey {
            self.apiKey = apiKey
        } else {
            self.apiKey = KeychainHelper.getPassword(for: "OpenSubtitlesAPIKey", service: "StreamHaven.API")
        }
        if self.apiKey == nil {
            PerformanceLogger.logNetwork("OpenSubtitles API Key not found in Keychain. Disabled.")
        }
    }
