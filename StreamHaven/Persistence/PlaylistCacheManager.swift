...
public final class PlaylistCacheManager {
    ...
    public static func getCachedPlaylist(url: URL, context: NSManagedObjectContext) -> Data? {
        ...
        let fetchRequest: NSFetchRequest<PlaylistCache> = PlaylistCache.fetchRequest()
        let urlPredicate = NSPredicate(format: "url == %@", url.absoluteString)
        let twentyFourHoursAgo = Date().addingTimeInterval(-24 * 60 * 60)
        let freshPredicate = NSPredicate(format: "lastRefreshed > %@", twentyFourHoursAgo as NSDate)
        fetchRequest.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: [urlPredicate, freshPredicate])
